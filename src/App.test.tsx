...
describe('App', () => {
  ...
  it('displays compact developer info header', () => {
    ...
    expect(compactHeader).toHaveTextContent('🔧 DEV: v.abc123 | Development | Unknown')
  })

  it('shows detailed view is initially hidden', () => {
    render(<App />)
    expect(screen.queryByTestId('dev-info-detailed-view')).not.toBeInTheDocument()
  })
  ...
  it('shows correct environment for different branches', async () => {
    ...
    expect(screen.getByTestId('dev-info-compact-header')).toHaveTextContent('Production')
  ...
  it('shows test environment for test branch', async () => {
    ...
    // Check compact header
    expect(screen.getByTestId('dev-info-compact-header')).toHaveTextContent('Test')
  ...
  it('shortens long commit hashes in compact view', () => {
    global.__GIT_INFO__ = { commit: 'abcdef1234567890', branch: 'test', date: '2025-09-10' }
    render(<App />)
