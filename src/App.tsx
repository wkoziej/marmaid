...
function DevInfoHeader({ gitInfo, environment, databaseEnv, supabaseUrl }: DevInfoProps) {
  ...
  const shortCommit = gitInfo.commit.length > 7 ? gitInfo.commit.substring(0, 7) : gitInfo.commit
  const shortDbInfo = databaseEnv.replace(' Database', '').replace(' Development', ' Dev')

  return (
    <div onKeyDown={handleKeyDown} tabIndex={0} data-testid="dev-info-container">
      {/* Compact header */}
      <div 
      ...
      {isExpanded && (
        <div className="dev-info-detailed" data-testid="dev-info-detailed-view" 
             style={{ 
               marginBottom: '1rem',
               maxWidth: '100%',
               overflowX: 'auto'
          ...
          <div style={{ 
            ...
            <p data-testid="version-display" style={{ margin: '0.2em 0' }}>
              <strong>Version:</strong> <span style={{ fontFamily: 'monospace' }}>{gitInfo.commit}</span>
            </p>
            <p data-testid="environment-display" style={{ margin: '0.2em 0' }}>
              <strong>Environment:</strong> {environment}
            </p>
            <p data-testid="branch-display" style={{ margin: '0.2em 0' }}>
              <strong>Branch:</strong> <span style={{ fontFamily: 'monospace' }}>{gitInfo.branch}</span>
            </p>
            ...
            <p data-testid="database-url-display" style={{ 
              margin: '0.2em 0',
              fontFamily: 'monospace',
              fontSize: '0.85em',
              wordBreak: 'break-all'
            }}>
              <strong>DB URL:</strong> {supabaseUrl.replace('https://', '').replace('.supabase.co', '')}
            </p>
            <p style={{ 
